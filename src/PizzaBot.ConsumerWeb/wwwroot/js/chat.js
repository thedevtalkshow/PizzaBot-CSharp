        ...
        if (response.ok) {
            ...
            response.text().then(text => {
                try {
                    ...
                    // strip trailing slash to avoid double-slash in the endpoint path
                    const privateEndpointBase = speechPrivateEndpoint?.replace('https://', '').replace(/\/$/, '')
                    const sttEndpoint = speechPrivateEndpoint
                        ? `wss://${privateEndpointBase}/stt/speech/universal/v2`
                        : `wss://${speechRegion}.stt.speech.microsoft.com/speech/universal/v2`
                    console.log(`[STT] Region: ${speechRegion}`)
                    console.log(`[STT] Private endpoint header: ${speechPrivateEndpoint ?? '(none)'}`)
                    console.log(`[STT] Resolved STT endpoint: ${sttEndpoint}`)
                    console.log(`[STT] Token present: ${!!speechToken}, length: ${speechToken?.length ?? 0}, prefix: ${speechToken?.substring(0, 10) ?? '(empty)'}`)

                    const speechRecognitionConfig = SpeechSDK.SpeechConfig.fromEndpoint(new URL(sttEndpoint))
                    ...
                    // log any cancellation/auth errors that would otherwise be silent
                    speechRecognizer.canceled = (s, e) => {
                        console.error(`[STT] Recognition canceled. Reason: ${SpeechSDK.CancellationReason[e.reason]}`)
                        if (e.reason === SpeechSDK.CancellationReason.Error) {
                            console.error(`[STT] Error code: ${e.errorCode}, details: ${e.errorDetails}`)
                        }
                    }

                    console.log('[STT] Speech recognizer created successfully.')
                } catch (err) {
                    console.error('[STT] Failed to create speech recognizer:', err)
                }
            })
        } else {
...
window.microphone = () => {
    ...
    if (!speechRecognizer) {
        console.error('[STT] Speech recognizer is not ready yet. Check earlier logs for initialization errors.')
        return
    }
...
// Expose for external callers (e.g. Blazor page send button)
